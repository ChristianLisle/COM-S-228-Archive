...
public class Plot {
	...
	private static class MyPanel extends JPanel {
		...
		/**
		 * This method draws the x- and y-axes with the origin at the pixel position
		 * (ox, oy).
		 * 
		 * @param g2
		 */
		private void drawAxes(Graphics2D g2) {
			...
			for (int i = MIN_XY; i <= MAX_XY; i++) {
				...
				if (i == 0) ;
					...
					if (i < 0) g2.drawString(String.valueOf(i), px - 9, oy + 15);
					else g2.drawString(String.valueOf(i), px - 6, oy + 15);
				}
				else if (i % 5 == 0) g2.drawLine(px, oy, px, oy - 4);
			}
			...
			for (int i = MIN_XY; i <= MAX_XY; i++) {
				...
				if (i == 0) ;
					...
					if (i < 0) g2.drawString(String.valueOf(-i), ox - 20, py + 5);
					else g2.drawString(String.valueOf(-i), ox - 25, py + 5);
				}
				else if (i % 5 == 0) g2.drawLine(ox, py, ox + 4, py);
			}
		}
		...
		/**
		 * x- or y-coordinate of the center pixel in the square representing a point
		 * with Cartesian x-coordinate or y-coordinate i.
		 * 
		 * @param i
		 * @return
		 */
		private int pixelCenter(int i) {
			if (i < -MAX_XY || i > MAX_XY) return WINDOW_WIDTH + 1;

			return ox + i * POINT_WIDTH;
		}

		/**
		 * minimum x- or y-coordinate of any pixel in the square representing a point
		 * with Cartesian x- or y-coordinate i.
		 * 
		 * @param i
		 * @return
		 */
	...
	public static void myFrame(final Point[] pts, final Segment[] segs, final String name) {
		if (map.containsKey(name)) map.get(name).dispose();
